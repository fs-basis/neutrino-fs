...
void CChannelList::paintItem(int pos, const bool firstpaint)
{
	...
	if(curr < (*chanlist).size()) {
		...
		if (!p_event->description.empty())
		{
			...
			{
				// align right
				g_Font[SNeutrinoSettings::FONT_TYPE_CHANNELLIST_DESCR]->RenderString(x + width - SCROLLBAR_WIDTH - offset_right - ch_desc_len, ypos + fheight, ch_desc_len, p_event->description, dcolor);
			}
			else
			{
				// align left
				g_Font[SNeutrinoSettings::FONT_TYPE_CHANNELLIST_DESCR]->RenderString(x + OFFSET_INNER_MID + numwidth + OFFSET_INNER_MID + prg_offset + OFFSET_INNER_MID + ch_name_len, ypos + fheight, ch_desc_len, p_event->description, dcolor);
			}
		}
		else
		{
