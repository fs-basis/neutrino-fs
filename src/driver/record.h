...
class CRecordInstance
{
	...
	public:
		...
		int tshift_mode;
		void SetStopMessage(const char *text)
		{
			rec_stop_msg = text;
		} ;
		int  GetStatus();
...
class CRecordManager : public CMenuTarget /*, public CChangeObserver*/
{
	...
	public:
		...
		void SetTimeshiftDirectory(std::string directory)
		{
			TimeshiftDirectory	= directory;
		};
		bool RecordingStatus(const t_channel_id channel_id = 0);
		bool TimeshiftOnly();
		bool Timeshift()
		{
			return (autoshift || shift_timer);
		};
