...
int CScanSetup::showScanMenu()
{
	...
	if (CFEManager::getInstance()->haveCable()) {
		...
		CMenuWidget * manualScan = new CMenuWidget(LOCALE_SATSETUP_MANUAL_SCAN, NEUTRINO_ICON_SETTINGS, w/*width*/, MN_WIDGET_ID_SCAN_MANUAL_SCAN);
		addScanMenuManualScan(manualScan);
		mf = new CMenuDForwarder(LOCALE_SATSETUP_MANUAL_SCAN, true, NULL, manualScan, "", have_other ? CRCInput::convertDigitToKey(shortcut++) : CRCInput::RC_green);
		mf->setHint("", LOCALE_MENU_HINT_SCAN_MANUAL);
		settings->addItem(mf);
		//simple cable scan
		...
		CMenuForwarder * fcableScan = new CMenuDForwarder(LOCALE_SATSETUP_CABLE, true, NULL, cableScan, "", have_other ? CRCInput::convertDigitToKey(shortcut++) : CRCInput::RC_yellow);
