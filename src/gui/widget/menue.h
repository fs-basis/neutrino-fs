...
class CMenuItem : public  CComponentsSignals
{
	...
	public:
		...
		virtual int getYPosition(void) const
		{
			return y;
		}
		...
		virtual int isMenueOptionChooser(void) const
		{
			return 0;
		}
		void setHint(const char *const icon, const neutrino_locale_t text)
		{
			hintIcon = (icon && *icon) ? icon : NULL;
			hint = text;
		}
		void setHint(const char *const icon, const std::string &text)
		{
			hintIcon = (icon && *icon) ? icon : NULL;
			hintText = text;
		}
...
class CMenuForwarder : public CMenuItem
{
	...
	public:
		...
		int paint(bool selected = false);
		int getWidth(void);
		neutrino_locale_t getTextLocale() const
		{
			return name;
		}
		CMenuTarget *getTarget() const
		{
			return jumpTarget;
		}
		const char *getActionKey()
		{
			return actionKey.c_str();
		}
...
class CMenuOptionNumberChooser : public CAbstractMenuOptionChooser
{
	...
	public:
		...
		int exec(CMenuTarget *parent);
		int isMenueOptionChooser(void) const
		{
			return 1;
		}
		int getWidth(void);
		...
		void setNumberFormat(std::string(*fun)(int))
		{
			numberFormatFunction = fun;
		}
		void setNumericInput(bool _numeric_input)
		{
			numeric_input = _numeric_input;
		}
...
class CMenuOptionStringChooser : public CMenuItem
{
	...
	public:
		...
		void setOptions(std::vector<std::string> &vec)
		{
			options = vec;
		}
		...
		{
			return *optionValuePtr;
		}
		sigc::signal<void> OnAfterChangeOption;
		void rememberLastItem(bool remember = true)
		{
			hold_last_item = remember;
		}
};
...
class CMenuWidget : public CMenuTarget, public CComponentsSignals
{
	...
	public:
		...
		void setSelected(const int &Preselected)
		{
		...
		int getSelectedLine(void)const
		{
			return no_action ? -1 : selected;
		};
		void enableFade(bool _enable)
		{
			fade = _enable;
		};
		...
		void setMenuPosMode(int pos_mode)
		{
			w_pos_mode = pos_mode;
		}
};
...
class CPINProtection
{
	...
	public:
		...
		virtual void setHint(neutrino_locale_t Hint)
		{
			hint = Hint;
		};
};
...
{
	public:
		CMenuSelectorTarget(int *select)
		{
			m_select = select;
		};
